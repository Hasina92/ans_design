        ...
        fetch(`ajax/get_order_details.php?id=${commandeId}`)
            ...
            .then((data) => {
                ...
                const renderArticlesList = (articles) => {
                    if (!articles || articles.length === 0) return '<em>Aucun article listé</em>';
                    
                    // Conteneur avec scroll pour le responsive (évite que le modal soit trop long sur mobile)
                    let html = '<div class="articles-scroll-box" style="margin-top:5px; max-height:200px; overflow-y:auto; border:1px solid #eee; background:#fafafa; padding:8px; border-radius:4px;">';
                ...
                const contentHtml = `
                    <div class="modal-header">
                        <h2 class="modal-title">Détails de la Commande #${data.numero_commande || data.id}</h2>
                    </div>
                            ...
                            <div class="modal-panel">
                                ...
                                <div class="info-grid">
                                    ...
                                    <div style="grid-column: span 2; margin-top:5px;">
                                        ...
                                        ${renderArticlesList(data.articles_detailed_list)}
                                    </div>
                                    
                                    <div class="total-info-grid" style="margin-top:10px;">
                                        <span>Total : </span> 
                                        <strong>${parseFloat(data.total_ttc).toLocaleString('fr-FR')} AR</strong>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="modal-panel">
                                ...
                                <div class="info-grid">
                                    ...
                                    <select id="newStatusSelect" class="modal-input">
                                        ...
                                        <option value="En attente devis" ${data.statut === "En attente devis" ? "selected" : ""}>En attente devis</option>
                                ...
                                <div class="timeline">
                                            ...
                                            <small>
                                                ...
                                                <strong>${(function () {
                                                    switch (data.methode_paiement) {
                                                        ...
                                                        case "recuperation": return "Point de vente";
                                                        default: return data.methode_paiement || "Non définie";
                                                    }
                                                })()}</strong>
                                            </small>
                                            ${data.details_paiement ? `<br><small>Détails : ${data.details_paiement}</small>` : ""}
                                        </div>
                                    </div>
                                    ...
                                    <div class="timeline-item">
                                        <div class="timeline-content">
                                            ...
                                            <p style="margin:5px 0;">${data.adresse_livraison || "Non définie"}</p>
                                            <small>${data.code_postal || ""} ${data.ville || ""}</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
